...
We are using the google sheet for summarizing all the results, which link is as follows.
...
- **All-in-one**
    - Read-only Result that contains all the verified true positives
    - The table is dynamically generated from scripts based on tables **Github-Top-1k-0329** and **Github-Top-100-0331**
...
- **Github-Top-1k-0329**
    ...
    - The listed repo only means that there is a function that contains class pollution behavior. It does not guarantee that the vulnerability can be triggered remotely, locally, or through library APIs.
...
2. Function-level Verification
    1. Find the CodeQL SARIF file from GitHub and download the repo source code (TODO: I am considering putting all the results on a shared server)
...
3. External Triggering
    ...
    2. If it shows potential for exploitation:
        ...
        - You may need to write a small piece of application code to drive the framework.
