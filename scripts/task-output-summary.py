...
def parse_result_log(log_file):
  ...
  with open(log_file, 'r', encoding='utf-8') as f:
    for line in f:
      # Updated regex to allow hyphens in repository names
      match = re.search(r"INFO - ([\w-]+) - .+\.ql\.sarif:", line)
      if match:
        repo_name = match.group(1)
        flagged_repos.append(repo_name)
      else:
...
def generate_csv_output(flagged_repos, repo_metadata, output_file):
  ...
  flagged_repos_sorted = sorted(
    flagged_repos,
    key=lambda x: repo_metadata.get(x, {}).get("stargazers_count", -1),
    reverse=True
  )

  with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ["repo_name", "stars", "url", "Confirmed", "CodeQL"]
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

    writer.writeheader()
    for repo_name in flagged_repos_sorted:
