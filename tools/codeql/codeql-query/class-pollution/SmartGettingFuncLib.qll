...
module TrackingParamToSmartGettingOpFlow = TaintTracking::GlobalWithState<TrackingParamToSmartGettingOpConfiguration>;
module Flow = TrackingParamToSmartGettingOpFlow; // For shortening the name

predicate isFuncParamter(TrackingParamToSmartGettingOpFlow::PathNode node) {
  exists(Parameter param|
    (param instanceof Name and node.getNode().asExpr() = param) or
    (param instanceof Tuple and node.getNode().asExpr() = param.(Tuple).getAnElt())
  )
}
...
predicate isSmartGettingFunction_ (Flow::PathNode sourceObjToItem, Flow::PathNode sourceKeyToItem, Flow::PathNode sourceObjToAttr, Flow::PathNode sourceKeyToAttr, Flow::PathNode getItemObj, Flow::PathNode getItemKey, Flow::PathNode getAttrObj, Flow::PathNode getAttrKey) {
  ...
  (
    isFuncParamter(sourceObjToItem) and isFuncParamter(sourceKeyToItem) and
    isFuncParamter(sourceObjToAttr) and isFuncParamter(sourceKeyToAttr)
  ) and 
  isGetItemOp(getItemObj.getNode().asExpr(), getItemKey.getNode().asExpr(), _) and
  isGetattrCall(getAttrObj.getNode().asExpr(), getAttrKey.getNode().asExpr(), _) and
  (
