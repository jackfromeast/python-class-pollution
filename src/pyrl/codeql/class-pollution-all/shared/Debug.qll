...
predicate selectUnresolvedCallNode(CallNode callNode) {
  not exists (Function func | resolvedCall(callNode, func)) and
  not exists (API::CallNode apiNode | apiNode.asCfgNode() = callNode)
}
...
predicate resolvedSimpleCall(ControlFlowNode call, FunctionObject callable){
